...
public class DevelopmentDataSeeder implements ApplicationRunner {
    ...
    private void seedVideoDeliveries(){
        if(Boolean.TRUE.equals(jdbc.queryForObject("select exists(select 1 from audit_logs where action='DEV_VIDEO_DELIVERY_DATA_SEEDED')",Boolean.class)))return;
        jdbc.update("insert into video_delivery_batches(class_id,video_type,target_month,title,description,due_date,status,created_by_employee_id,note) select c.id,'MONTHLY_PERSONAL_VIDEO',date '2026-07-01','July speaking videos','Direct Zalo delivery tracking',date '2026-07-15','ACTIVE',e.id,'Development seed' from classes c,employees e where c.class_code='CLS001' and e.employee_code='EMP002'");
        jdbc.update("insert into video_delivery_batches(class_id,video_type,title,description,due_date,status,created_by_employee_id,note) select c.id,'FINAL_COURSE_VIDEO','Final course videos','Direct Zalo delivery tracking',date '2026-07-20','ACTIVE',e.id,'Development seed' from classes c,employees e where c.class_code='CLS002' and e.employee_code='EMP002'");
        jdbc.update("with targets as (select b.id batch_id,b.class_id,b.video_type,b.target_month,b.title,ce.student_id,sp.parent_id,row_number() over(partition by b.id order by s.student_code) rn from video_delivery_batches b join class_enrollments ce on ce.class_id=b.class_id and ce.status='ACTIVE' and ce.deleted_at is null join students s on s.id=ce.student_id left join lateral(select x.parent_id from student_parents x where x.student_id=s.id and x.deleted_at is null and x.can_receive_notification=true order by x.is_primary_contact desc nulls last limit 1)sp on true where b.note='Development seed') insert into student_video_deliveries(batch_id,student_id,class_id,parent_id,video_type,target_month,title,message_content,channel,status,assigned_to_employee_id,sent_by_employee_id,sent_at,failed_reason,note) select t.batch_id,t.student_id,t.class_id,t.parent_id,t.video_type,t.target_month,t.title,'APIX English gửi phụ huynh video của bé. Video được gửi trực tiếp trong tin nhắn Zalo này.','MANUAL_ZALO_DIRECT',case when t.video_type='FINAL_COURSE_VIDEO' and t.rn=1 then 'FAILED' when t.rn=1 then 'PENDING' when t.rn=2 then 'PREPARED' when t.rn=3 then 'OPENED_ZALO' else 'SENT_MANUALLY' end,e.id,case when t.rn>3 then e.id end,case when t.rn>3 then now() end,case when t.video_type='FINAL_COURSE_VIDEO' and t.rn=1 then 'Parent phone temporarily unavailable' end,'Development seed' from targets t,employees e where e.employee_code='EMP002' on conflict(batch_id,student_id) do nothing");
        jdbc.update("insert into video_delivery_attempts(video_delivery_id,action,actor_employee_id,parent_id,recipient_phone,message_content,note) select d.id,case d.status when 'PREPARED' then 'PREPARED_MESSAGE' when 'OPENED_ZALO' then 'OPENED_ZALO' when 'SENT_MANUALLY' then 'MARKED_SENT' when 'FAILED' then 'MARKED_FAILED' end,e.id,d.parent_id,p.phone,d.message_content,'Development seed attempt' from student_video_deliveries d join video_delivery_batches b on b.id=d.batch_id join employees e on e.employee_code='EMP002' left join parents p on p.id=d.parent_id where b.note='Development seed' and d.status<>'PENDING'");
        jdbc.update("insert into audit_logs(actor_user_id,actor_employee_id,action,module,entity_type,after_data,created_at) select u.id,e.id,'DEV_VIDEO_DELIVERY_DATA_SEEDED','system','DevelopmentSeed',jsonb_build_object('batches',2),now() from users u join employees e on e.user_id=u.id where u.email='office1@apix.local'");
    }
